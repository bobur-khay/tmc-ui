...
//
// The deployment modes are driven entirely by build-time globals and
// Vite env variables that App.tsx reads at runtime:
//   - static                → __SERVER_AVAILABLE__ = false
//   - backend no auth       → __SERVER_AVAILABLE__ = true, SERVER_URL set, no TOKEN_URL
//   - backend with auth     → __SERVER_AVAILABLE__ = true, SERVER_URL + TOKEN_URL set
//
...
export function stubDeployGlobals({
  ...
  vi.stubGlobal('__EDITDOR_URL__', viteEditdorUrl ?? env.EDITDOR_URL ?? '');
  vi.stubGlobal('__PLAYGROUND_URL__', vitePlaygroundUrl ?? env.PLAYGROUND_URL ?? '');
  vi.stubGlobal('__TOKEN_URL__', viteTokenUrl ?? env.TOKEN_URL ?? '');
  vi.stubGlobal('__SERVER_URL__', viteServerUrl ?? env.SERVER_URL ?? '');
  vi.stubGlobal(
    '__CREDENTIALS_SETUP_MESSAGE__',
    viteSetupCredentialsMessage ?? env.CREDENTIALS_SETUP_MESSAGE ?? '',
  );
