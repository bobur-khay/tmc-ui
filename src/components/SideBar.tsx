...
interface SideBarProps {
  ...
  onChangedSinceChange: (value: string) => void;
...
const SideBar: React.FC<SideBarProps> = ({
  ...
  changedSince,
  onFilterChange,
  onChangedSinceChange,
  onAddProtocol,
}) => {
  const [showScrollTop, setShowScrollTop] = useState(false);
  const [changedSinceDraft, setChangedSinceDraft] = useState(changedSince);
  ...
  useEffect(() => {
    setChangedSinceDraft(changedSince);
  }, [changedSince]);
        ...
        <div className="flex flex-col gap-x-8 gap-y-10">
          ...
          <form className="lg:block">
            ...
            {__DEPLOY_TYPE__ === 'SERVER_AVAILABLE' && (
              <Disclosure as="div" className="border-b border-border-subtle py-6">
                <h3 className="flow-root">
                  <DisclosureButton className="group flex w-full items-center justify-between bg-surface-canvas py-3 text-sm">
                    <span className="font-medium text-text-secondary">Changed Since</span>
                    <span className="ml-6 flex items-center">
                      <PlusIcon
                        aria-hidden="true"
                        className="size-5 text-icon-brand group-hover:text-interactive-hover group-data-[open]:hidden"
                ...
                <DisclosurePanel className="bg-surface-canvas pt-6">
                  <label
                    htmlFor="filter-changed-since"
                    className="mb-2 block text-sm text-text-primary"
                  >
                    Show TMs changed on or after
                  </label>
                  <Input
                    id="filter-changed-since"
                    type="date"
                    value={changedSinceDraft}
                    onChange={(event) => setChangedSinceDraft(event.target.value)}
                  />
                  <Button
                    type="button"
                    text="Apply"
                    onClick={() => onChangedSinceChange(changedSinceDraft)}
                    disabled={changedSinceDraft === changedSince}
                    className="mt-3 w-full justify-center border"
                    variant="default"
                  />
                </DisclosurePanel>
              </Disclosure>
